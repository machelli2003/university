...
async def init_db():
    ...
    try:
        ...
        print(f"✓ Database indexes created: {len(index_results['created'])} new, {len(index_results['already_exist'])} existing")
        if index_results['errors']:
            print(f"⚠ Index errors (non-fatal): {index_results['errors']}")
